...
export default function StoreOrderDetailPage() {
  ...
  if (!order || !storeStatuses.includes(order.status))
    return (
      <div className="p-6">
        <Link href="/store/orders" className="underline">
          Back to store orders
        </Link>
        <p className="mt-8 text-sm text-[var(--muted)]">
          This order is not currently in the store fulfilment workflow.
        </p>
      </div>
    );
  ...
  async function updateStatus(status: OrderStatus) {
    ...
    try {
      ...
      await refresh();
      setMessage("Order and shipment workflow updated in Sanity.");
    } catch (cause) {
  ...
  return (
    <div className="min-h-full bg-[var(--paper-2)]">
      ...
      <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[1.4fr_0.8fr] lg:p-8">
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-[10px] text-[var(--muted)]">
                      Live stock:{" "}
                      {product ? product.onHand : "Product not found"}
                    </p>
                  </div>
                  <p className="text-sm font-semibold">
                    {formatKes(line.quantity * line.unitPrice)}
                  </p>
                </div>
              );
            })}
        ...
        <aside className="rounded-xl border hairline bg-[var(--paper)] p-5">
          <p className="kicker text-[var(--muted)]">Fulfilment status</p>
          <div className="relative mt-4">
            <select
              disabled={saving}
              value={order.status}
              onChange={(e) => void updateStatus(e.target.value as OrderStatus)}
              className="w-full appearance-none rounded-lg border hairline bg-[var(--paper)] pl-3 pr-10 py-2.5 text-sm outline-none transition focus:border-[var(--deep-green)] disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          <p className="mt-4 text-xs leading-6 text-[var(--muted)]">
            Changes are persisted to Sanity and synchronised with the related
            shipment.
          </p>
        </aside>
      </div>
