...
export default function AdminProductDetailPage() {
  ...
  async function save() {
    ...
    setMessage(null);
    try {
      await mutateBackoffice(`/api/backoffice/products/${encodeURIComponent(liveProductId)}`, {
        onHand: draft!.onHand,
    ...
    <div className="min-h-full bg-[var(--paper-2)]">
      <div className="border-b hairline bg-[var(--paper)] px-4 py-6 sm:px-6 lg:px-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Link href="/admin/products" className="group mb-3 inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
          ...
          <div className="flex gap-2">
            {!editing ? (
              <button type="button" onClick={() => setEditing(true)} className="inline-flex items-center gap-2 rounded-full border hairline px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.08em]"><Pencil size={14} /> Edit live values</button>
            ) : (
              <>
                <button type="button" disabled={saving} onClick={() => void save()} className="inline-flex items-center gap-2 rounded-full bg-[var(--ink)] px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.08em] !text-white disabled:opacity-50"><Check size={14} /> {saving ? "Saving…" : "Save"}</button>
                <button type="button" onClick={() => { setDraft(product); setEditing(false); }} className="inline-flex items-center gap-2 rounded-full border hairline px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.08em]"><X size={14} /> Cancel</button>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 p-4 sm:p-6 lg:grid-cols-[1fr_1.5fr] lg:p-8">
        <div className="space-y-4">
          ...
          <div className="rounded-xl border hairline bg-[var(--paper)] p-5 sm:p-6">
            ...
            {editing ? (
              <input value={draft.location} onChange={(e) => setDraft({ ...draft, location: e.target.value })} className="mt-3 w-full rounded-lg border hairline px-3 py-2.5 text-sm outline-none" />
            ) : (
        ...
        <div className="rounded-xl border hairline bg-[var(--paper)] p-5 sm:p-6">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            {([
              ...
              <div key={field} className="rounded-lg border hairline p-4">
                ...
                {editing ? (
                  <input type="number" min={0} value={draft[field]} onChange={(e) => setDraft({ ...draft, [field]: Number(e.target.value) })} className="mt-2 w-full rounded-md border hairline px-2 py-2 text-sm outline-none" />
                ) : (
          ...
          <div className="mt-6 grid gap-4 border-t hairline pt-5 sm:grid-cols-3">
            ...
            <div>
              ...
              {editing ? <input type="number" min={0} value={draft.retailPrice} onChange={(e) => setDraft({ ...draft, retailPrice: Number(e.target.value) })} className="mt-2 w-full rounded-md border hairline px-2 py-2 text-sm outline-none" /> : <p className="mt-2 text-lg font-semibold">{formatKes(product.retailPrice)}</p>}
            </div>
            <div>
