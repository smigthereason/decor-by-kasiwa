  ...
  return (
    <div className="min-h-full bg-[var(--paper-2)]">
      <div className="border-b hairline bg-[var(--paper)] px-4 py-6 sm:px-6 lg:px-10">
        ...
        <h1 className="mt-2 text-2xl font-medium tracking-[-0.04em] sm:text-3xl lg:text-4xl">Live product catalogue</h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[var(--muted)]">
          This is the same Sanity catalogue used by the customer shop. There are no demo products in this view.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              className="w-full rounded-full border hairline bg-[var(--paper)] py-2.5 pl-9 pr-4 text-sm outline-none focus:border-[var(--ink)]"
            />
          </div>
          <select
            value={categoryFilter}
            onChange={(event) => setCategoryFilter(event.target.value)}
            className="min-h-10 rounded-full border hairline bg-[var(--paper)] px-4 text-sm outline-none"
          >
            {categories.map((category) => <option key={category}>{category}</option>)}
          </select>
          <span className="sm:ml-auto text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
                  ...
                  {filteredProducts.map((product) => (
                    <tr key={product.id} className="border-b hairline last:border-b-0">
                      ...
                      <td className="px-4 py-4 text-right">
                        <Link href={`/admin/products/${encodeURIComponent(product.id)}`} className="inline-grid size-8 place-items-center rounded-full border hairline" aria-label={`View ${product.name}`}>
                          <Eye size={13} />
                        </Link>
                      </td>
                    </tr>
