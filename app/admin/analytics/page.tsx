...
export default function AdminAnalyticsPage() {
  ...
  const topCustomers = [...data.customers].sort((a,b) => b.lifetimeValue - a.lifetimeValue).slice(0,8);
  const exportRows = [
    { section: "Business totals", metric: "Paid revenue", value: revenue, detail: `${paidOrders.length} paid orders` },
    { section: "Business totals", metric: "Average order", value: avgOrder, detail: "Paid orders" },
    { section: "Business totals", metric: "Customers", value: data.customers.length, detail: "Live customer records" },
  ...
  return (
    <div className="min-h-full bg-[var(--paper-2)]">
      <div className="border-b hairline bg-[var(--paper)] px-4 py-6 sm:px-6 lg:px-10"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="kicker text-[var(--muted)]">Analytics</p><h1 className="mt-2 text-2xl font-medium tracking-[-0.04em] sm:text-3xl lg:text-4xl">Live business analytics</h1><p className="mt-3 text-sm text-[var(--muted)]">Calculated only from live Sanity products, customers and orders.</p></div><ExportButtons title="Decor by Kasiwa Analytics" columns={[{key:"section",label:"Section"},{key:"metric",label:"Metric"},{key:"value",label:"Value"},{key:"detail",label:"Detail"}]} rows={exportRows}/></div></div>
