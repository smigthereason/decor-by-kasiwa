  ...
  async function save() {
    ...
    try {
      await mutateBackoffice(
        `/api/backoffice/customers/${encodeURIComponent(customerId)}`,
        payload,
      );

      await refresh();

      setMessage("Customer profile updated in Sanity.");
    } catch (cause) {
      setSaveError(
      ...
      <div className="border-b hairline bg-[var(--paper)] px-4 py-6 sm:px-6 lg:px-10">
        ...
        <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-[10px] uppercase tracking-[0.08em] text-[var(--muted)]">
              {customer.authenticated
                ...
                : "Guest purchaser"}
              {" · "}
              {customer.source === "GUEST_CHECKOUT"
          ...
          <div className="flex flex-wrap gap-2">
            <StatusPill
              value={(customer.status || "ACTIVE").toLowerCase()}
            />
            <StatusPill
              value={(customer.role || "CUSTOMER").toLowerCase()}
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[0.85fr_1.5fr] lg:p-8">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-xl border hairline bg-[var(--paper)] p-5">
            <p className="kicker text-[var(--muted)]">Contact</p>
          ...
          <section className="rounded-xl border hairline bg-[var(--paper)] p-5">
            ...
            <p className="mt-2 text-[11px] leading-relaxed text-[var(--muted)]">
              Customer contact information, access level and account status
              are persisted directly to Sanity.
            </p>
              ...
              <label className="grid gap-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none rounded-lg border hairline bg-[var(--paper)] pl-3 pr-10 py-2.5 text-sm font-normal normal-case tracking-normal text-[var(--ink)] outline-none transition focus:border-[var(--deep-green)]"
                  >
          ...
          {/* COMMERCE */}
          <section className="rounded-xl border hairline bg-[var(--paper)] p-5">
            <p className="kicker text-[var(--muted)]">Commerce</p>
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-xs text-[var(--muted)]">
                Live purchases associated with this customer&apos;s email address.
              </p>
            </div>
