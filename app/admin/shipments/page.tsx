...
export default function AdminShipmentsPage() {
  ...
  const statuses = useMemo(
    () => ["All", ...Array.from(new Set(shipments.map((item) => item.status)))],
    [shipments],
  );
  ...
  return (
    <div className="min-h-full min-w-0 bg-[var(--paper-2)]">
      <div className="border-b hairline bg-[var(--paper)] px-4 py-6 sm:px-6 lg:px-10">
        <p className="kicker text-[var(--muted)]">Shipments</p>
        <h1 className="mt-2 text-2xl font-medium tracking-[-0.04em] sm:text-3xl lg:text-4xl">Live fulfilment shipments</h1>
      </div>
      ...
      <section className="p-4 sm:p-6 lg:p-8">
        {filtered.length ? (
          <>
            <div className="grid gap-3 md:hidden">
              {filtered.map((shipment) => (
                <article key={shipment.id} className="min-w-0 rounded-xl border hairline bg-[var(--paper)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-xs font-semibold">{shipment.shipmentNumber}</p>
                      <p className="mt-1 truncate text-sm font-medium">{shipment.customerName}</p>
                      <p className="mt-1 truncate text-[10px] text-[var(--muted)]">{shipment.orderNumber}</p>
                    </div>
                    <StatusPill value={shipment.status} />
                  </div>
                  <p className="mt-4 break-words text-xs text-[var(--muted)]">{shipment.destination}</p>
                  <p className="mt-3 border-t hairline pt-3 text-[10px] text-[var(--muted)]">Updated {formatDateTime(shipment.updatedAt)}</p>
                </article>
              ))}
            </div>

            <div className="hidden max-w-full overflow-hidden rounded-xl border hairline bg-[var(--paper)] md:block">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[850px] text-left">
                  <thead>
                    <tr className="border-b hairline">
                      {["Shipment", "Order", "Customer", "Destination", "Updated", "Status"].map((heading) => (
                  ...
                  <tbody>
                    {filtered.map((shipment) => (
                      <tr key={shipment.id} className="border-b hairline last:border-0">
                        <td className="px-4 py-4 text-xs font-semibold">{shipment.shipmentNumber}</td>
                        <td className="px-4 py-4 text-xs">{shipment.orderNumber}</td>
                        <td className="px-4 py-4 text-xs">{shipment.customerName}</td>
                        <td className="px-4 py-4 text-xs text-[var(--muted)]">{shipment.destination}</td>
                        <td className="px-4 py-4 text-[10px] text-[var(--muted)]">{formatDateTime(shipment.updatedAt)}</td>
                        <td className="px-4 py-4"><StatusPill value={shipment.status} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="rounded-xl border hairline bg-[var(--paper)] p-12 text-center">
            ...
            <p className="mt-4 text-sm font-medium">No live shipments found</p>
          </div>
        )}
      </section>
    </div>
