  ...
  async function update(values: {
    ...
    try {
      await mutateBackoffice(
        `/api/backoffice/orders/${encodeURIComponent(liveOrderId)}`,
        values,
      );
      await refresh();
        ...
        <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">
              {formatDateTime(order.createdAt)} · {order.deliveryLocation}
            </p>
          </div>
      ...
      <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[1.4fr_0.8fr] lg:p-8">
        ...
        <aside className="space-y-4">
          <section className="rounded-xl border hairline bg-[var(--paper)] p-5">
            ...
            <p className="mt-1 text-xs text-[var(--muted)]">
              {order.customerPhone || "No phone supplied"}
            </p>
          </section>
          <section className="rounded-xl border hairline bg-[var(--paper)] p-5">
            <p className="kicker text-[var(--muted)]">Live status</p>

            <label className="mt-4 block text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
              Order status
            </label>
            <div className="relative mt-2 w-full">
              <select
                ...
                value={order.status}
                onChange={(e) =>
                  void update({ status: e.target.value as OrderStatus })
                }
                className="w-full cursor-pointer appearance-none rounded-lg border hairline bg-[var(--paper)] pl-3 pr-10 py-2.5 text-sm font-medium outline-none transition-colors focus:border-[var(--deep-green)] focus:ring-2 focus:ring-[var(--deep-green)]/15 disabled:cursor-not-allowed disabled:opacity-50"
              >
            ...
            <div className="relative mt-2 w-full">
              <select
                ...
                onChange={(e) =>
                  void update({ paymentStatus: e.target.value as PaymentStatus })
                }
                className="w-full cursor-pointer appearance-none rounded-lg border hairline bg-[var(--paper)] pl-3 pr-10 py-2.5 text-sm font-medium outline-none transition-colors focus:border-[var(--deep-green)] focus:ring-2 focus:ring-[var(--deep-green)]/15 disabled:cursor-not-allowed disabled:opacity-50"
              >
            ...
            <p className="mt-4 flex items-center gap-2 text-[10px] text-[var(--muted)]">
              <Check size={12} /> Setting Ready for Store creates/updates the
              live shipment.
            </p>
          </section>
        </aside>
