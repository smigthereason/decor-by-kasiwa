    ...
    <div className="min-h-full bg-[var(--paper-2)]">
      ...
      <div className="border-b hairline bg-[var(--paper)] px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <ExportButtons title="Product Catalogue" columns={[{key:"name",label:"Product"},{key:"sku",label:"SKU"},{key:"category",label:"Category"},{key:"location",label:"Location"},{key:"onHand",label:"On Hand"},{key:"reserved",label:"Reserved"},{key:"incoming",label:"Incoming"},{key:"retailPrice",label:"Retail Price (KES)"},{key:"unitCost",label:"Procurement Cost (KES)"},{key:"available",label:"Shop Visible"}]} rows={filteredProducts.map((product)=>({name:product.name,sku:product.sku,category:product.category,location:product.location,onHand:product.onHand,reserved:product.reserved,incoming:product.incoming,retailPrice:product.retailPrice,unitCost:product.unitCost,available:product.available===false?"No":"Yes"}))}/>
        </div>
      </div>
