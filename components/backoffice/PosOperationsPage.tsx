...
type HistoryOrder = {
  ...
  subtotal: number; discountAmount: number; total: number; amountPaid: number; balanceDue: number; cashTendered: number; cashChangeDue: number; refundedAmount: number;
...
export default function PosOperationsPage({ basePath }: { basePath: "/admin" | "/store" }) {
  ...
  const exportData = useMemo(() => {
    if (tab === "history") return {
      title: "Sales History",
      columns: [{key:"order",label:"Sale"},{key:"customer",label:"Customer"},{key:"phone",label:"Phone"},{key:"channel",label:"Channel"},{key:"cashier",label:"Cashier"},{key:"total",label:"Total (KES)"},{key:"tendered",label:"Cash Tendered (KES)"},{key:"change",label:"Change Due (KES)"},{key:"paid",label:"Paid (KES)"},{key:"balance",label:"Balance (KES)"},{key:"refunded",label:"Refunded (KES)"},{key:"status",label:"Status"},{key:"soldAt",label:"Sold At"}],
      rows: orders.map((order) => ({order:order.orderNumber,customer:order.customerName,phone:order.customerPhone,channel:`${order.salesChannel} / ${order.paymentChannel || "—"}`,cashier:order.soldByName,total:order.total,tendered:order.cashTendered,change:order.cashChangeDue,paid:order.amountPaid,balance:order.balanceDue,refunded:order.refundedAmount,status:order.paymentStatus,soldAt:order.soldAt})),
    };
    if (tab === "reports" && report) {
      ...
      return {
        ...
        rows: [
          {section:"TOTALS",name:"Total sales",orders:report.orders,quantity:"",amount:report.totalSales},
          {section:"TOTALS",name:"Net sales after refunds",orders:"",quantity:"",amount:Math.max(0,report.totalSales-report.refunds)},
          {section:"TOTALS",name:"Average order",orders:"",quantity:"",amount:averageOrder},
          ...
          {section:"PAYMENT",name:"Cash",orders:"",quantity:"",amount:report.cashPayments},
